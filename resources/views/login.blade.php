...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="antialiased">
        <div class="relative flex  justify-center min-h-screen bg-gray-100 dark:bg-gray-900 sm:items-center py-4 sm:pt-0">
            <button id="login" type="button" value="Click to login " onclick="getToken()" style="min-width: 300px" class="btn btn-primary  btn-sm">Click to Authorize</button>

            <form class="m-5" method="POST" action="{{ route('login') }}">
                @csrf
                <input type="hidden" class="form-control" id="token" name="token" value="" required >
                <input type="submit" id="submit"  style="min-width: 300px" class="btn btn-secondary btn-sm btn-lg disabled " value="login" disabled required>
            </form>
        ...
        function getToken() {
            ...
            url += "&flags=" + 0x3;			// options, 0x1 = add username in response
